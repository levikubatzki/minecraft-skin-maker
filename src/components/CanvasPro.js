...
export default function CanvasPro({ tool, color, size, mirror, grid, layer, baseImg, overlayImg, setBaseImg, setOverlayImg, onSnapshot }) {
    ...
    function setActive(img) { layer === 'base' ? setBaseImg(img) : setOverlayImg(img); }
    function rgba(c) { const s = c.replace('#', ''); const v = s.length === 3 ? s.split('').map(c => c + c).join('') : s; const n = parseInt(v, 16); return [(n >> 16) & 255, (n >> 8) & 255, n & 255, 255]; }
    function drawPoint(img, x, y, rgbaVal) {
        if (x < 0 || y < 0 || x >= W || y >= H)
            return;
    ...
    function handle(e) {
        ...
        if (tool === 'brush') {
            ...
            if (mirror)
                drawPoint(img, W - 1 - x, y, rgba(color));
        }
        else if (tool === 'eraser') {
            drawPoint(img, x, y, [0, 0, 0, 0]);
            if (mirror)
                drawPoint(img, W - 1 - x, y, [0, 0, 0, 0]);
        }
        else if (tool === 'fill') {
            floodFill(img, x, y, rgba(color));
            if (mirror)
                floodFill(img, W - 1 - x, y, rgba(color));
        }
        else if (tool === 'picker') {
            const i = (y * W + x) * 4;
            const d = img.data;
    ...
    return (_jsxs("div", { className: "neon-card p-3", children: [_jsxs("div", { className: "flex items-center justify-between mb-2", children: [_jsxs("div", { className: "font-semibold", children: ["Canvas (", W, "\u00D7", H, ")"] }), _jsxs("div", { className: "flex items-center gap-2", children: [_jsx("span", { className: "label", children: "Zoom" }), _jsx("input", { className: "range", type: "range", min: 4, max: 32, value: zoom, onChange: e => setZoom(parseInt(e.target.value)) }), hover && _jsxs("span", { className: "badge", children: ["x:", hover.x, " y:", hover.y] })] })] }), _jsx("div", { className: "overflow-auto", children: _jsx("canvas", { ref: canvasRef, onMouseDown: (e) => { onSnapshot(); handle(e); }, onMouseMove: handle, style: { imageRendering: 'pixelated', cursor: tool === 'picker' ? 'crosshair' : 'default', background: '#0b0b17', borderRadius: 12 } }) }), _jsx("p", { className: "text-neon-100 text-xs mt-1", children: "Tipp: + / - f\u00FCr Zoom, G = Grid, M = Mirror" })] }));
}
