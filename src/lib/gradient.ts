...
export function applyGradientToImageData(img: ImageData, opt: GradientOptions) {
  ...
  for (let y=0; y<height; y++) for (let x=0; x<width; x++) {
    ...
    if (opt.dither) {
      ...
      data[i]=clamp(r+threshold*32); data[i+1]=clamp(g+threshold*32); data[i+2]=clamp(bl+threshold*32);
    } else { data[i]=r; data[i+1]=g; data[i+2]=bl; }
    data[i+3]=255;
  }
}
function hexToRgb(hex: string): [number,number,number] {
  const s = hex.replace('#',''); const v = s.length===3 ? s.split('').map(c=>c+c).join('') : s;
  const n = parseInt(v,16); return [(n>>16)&255,(n>>8)&255,n&255];
}
function mix(a:[number,number,number], b:[number,number,number], t:number):[number,number,number] {
  const cl=(x:number)=>Math.round(x);
