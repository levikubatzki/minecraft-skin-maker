...
export function clone(img) { return new ImageData(new Uint8ClampedArray(img.data), img.width, img.height); }
...
export function makeDefaultSteve() {
    ...
    ctx.fillRect(16, 20, 8, 12); // body
    ctx.fillStyle = '#3AA3A3';
    ctx.fillRect(40, 20, 4, 12);
    ctx.fillRect(48, 20, 4, 12); // arms
    ctx.fillStyle = '#6B4F2A';
    ctx.fillRect(8, 52, 4, 12);
    ctx.fillRect(20, 52, 4, 12); // legs
    return ctx.getImageData(0, 0, W, H);
}
